#!/usr/bin/env bash
{
  ...
  BASENAME="$(basename $0)"; _R=$?; [[ $_R -ne 0 ]] && fail $_R "Failed: basename $0"
  NAME="${BASENAME%.*}"
  ...
  OPTS="--chmod=D750,F640 --delete --delete-excluded --delete-after -rltzRi --open-noatime --safe-links --timeout=3600"
  ...
  cmd() {
    if [[ $# -lt 2 ]]; then
      echo "Incorrect arguments: cmd <working diretory> <command> [arg1] ... [argN]"
      return 1
    fi
