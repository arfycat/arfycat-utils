#!/usr/bin/env bash
{
  ...
  RCLONE="$(dirname "$(realpath "${0}")")/rclone.sh"
  SRCDIR="/"
  DSTDIR="/"
  REMOTE="b2"
  ...
  case "$1" in
    cleanup)
      ...
      "${RCLONE}" -r "${REMOTE}" -d "${DSTDIR}/${CLEANUP}" -p "${BUCKET}${BUCKET_DIR}" cleanup
      exit $?
      ;;
    sync)
      "${RCLONE}" -s "${SRCDIR}" -d "${DSTDIR}" -r "${REMOTE}" -p "${BUCKET}${BUCKET_DIR}" -f "rclone.filter" -f "${REMOTE_FILTER}" sync
      exit $?
      ;;
    lsr)
      if [[ $# -ge 2 ]]; then LS="$2"; else LS="/"; fi
      "${RCLONE}" -d "${LS}" -r "${REMOTE}" -p "${BUCKET}${BUCKET_DIR}" lsr
      exit $?
      ;;
  esac
