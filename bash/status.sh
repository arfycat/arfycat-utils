...
echo '> df'
if [[ -v LINUX ]]; then
  timeout 5s df -hTx tmpfs
else
  timeout 5s df -hTt nonullfs,linprocfs,devfs,tmpfs,fdescfs,linsysfs,procfs,zfs
fi
echo
...
if [[ -x /home/flexfarmer/flexfarmer.sh ]]; then
  echo '> flexfarmer'
  timeout 5s /home/flexfarmer/flexfarmer.sh status
...
if [[ -x /home/phoenixminer/phoenixminer.sh ]]; then
  echo '> phoenixminer'
  timeout 5s /home/phoenixminer/phoenixminer.sh status
...
if [[ -x /home/lolminer/lolminer.sh ]]; then
  echo '> lolminer'
  timeout 5s /home/lolminer/lolminer.sh status
...
if [[ -x /home/raptoreum/cpuminer-gr-avx2.sh ]]; then
  ...
  timeout 5s /home/raptoreum/cpuminer-gr-avx2.sh status
